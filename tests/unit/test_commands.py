...
class TestGetStatus:
    @patch("gitux.git.commands.subprocess.run")
...
class TestGetUser:
    @patch("gitux.git.commands._try_config", return_value="hector")
    def test_name_set_returns_name(self, mock_config) -> None:
        assert get_user() == "hector"
        mock_config.assert_called_once_with("user.name")

    @patch("gitux.git.commands._try_config", side_effect=["", "hector@example.com"])
    @patch("gitux.git.commands._os_login", return_value="fallback")
    def test_empty_name_falls_to_email(self, mock_login, mock_config) -> None:
        assert get_user() == "hector@example.com"
        mock_login.assert_not_called()

    @patch("gitux.git.commands._try_config", return_value="")
    @patch("gitux.git.commands._os_login", return_value="hector")
    ...
    def test_oserror_falls_back_to_env(self, mock_config) -> None:
        ...
        with patch("gitux.git.commands.os", fake_os):
...
class TestGetOperationState:
    def test_merge_in_progress(self, tmp_path) -> None:
        ...
        with patch("gitux.git.commands.get_git_dir", return_value=str(tmp_path)):
